...
export default function KeyManagement() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (res) => {
      if (res.code === 0) { setRawKey((res.data as { raw_key: string; key: Key }).raw_key); toast({ title: '成功', description: 'Key 创建成功' }); queryClient.invalidateQueries({ queryKey: ['tenant-keys'] }) }
  ...
  const toggleDisableMutation = useMutation({
    ...
    onSuccess: (res) => { if (res.code === 0) { toast({ title: '成功', description: 'Key 已禁用' }); queryClient.invalidateQueries({ queryKey: ['tenant-keys'] }) } else toast({ title: '错误', description: res.message, variant: 'destructive' }) },
  })
  const toggleEnableMutation = useMutation({
    mutationFn: enableKey,
    onSuccess: (res) => { if (res.code === 0) { toast({ title: '成功', description: 'Key 已启用' }); queryClient.invalidateQueries({ queryKey: ['tenant-keys'] }) } else toast({ title: '错误', description: res.message, variant: 'destructive' }) },
  })
  const deleteMutation = useMutation({
    mutationFn: deleteKey,
    onSuccess: (res) => { if (res.code === 0) { toast({ title: '成功', description: 'Key 已删除' }); queryClient.invalidateQueries({ queryKey: ['tenant-keys'] }) } else toast({ title: '错误', description: res.message, variant: 'destructive' }); setDeleteTarget(null) },
  })
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={handleExportCSV}><Download className="mr-2 h-4 w-4" />导出 CSV</Button>
          <Button variant="outline" size="sm" onClick={handleExportJSON}><Download className="mr-2 h-4 w-4" />导出 JSON</Button>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>状态</TableHead><TableHead>创建时间</TableHead><TableHead>最后使用</TableHead><TableHead>操作</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={8} className="text-center text-muted-foreground">暂无 Key 数据</TableCell></TableRow>
            ) : (
                ...
                <TableRow key={key.id}>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      <Button variant="ghost" size="icon" onClick={() => { setSelectedKey({ id: key.id, alias: key.alias, remaining: key.remaining_amount }); setEditOpen(true) }}>
                        <Edit className="h-4 w-4" />
                      </Button>
                      {key.status === 'unused' ? <Button variant="ghost" size="icon" onClick={() => toggleDisableMutation.mutate(key.id)}><Ban className="h-4 w-4" /></Button> : null}
                      {key.status === 'disabled' ? <Button variant="ghost" size="icon" onClick={() => toggleEnableMutation.mutate(key.id)}><CheckCircle className="h-4 w-4" /></Button> : null}
                      <Button variant="ghost" size="icon" onClick={() => setDeleteTarget(key.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))
